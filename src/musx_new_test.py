...
class MUSX(object):
    ...
    def compute(self, prob_arr=None, coeff=1):
        """
        ...
        if not self.oracle.solve(assumptions=self.sels):
            ...
            approx = sorted(self.oracle.get_core()) # No need to sort? 
            # Get numpy array corresponding to approx and sort by probability
            
            if prob_arr:
                prob = []
                for i in range(len(approx)):
                    prob.append(prob_arr[approx[i]-1] + (approx[i]-self.org_topv)/self.clause_amt*coeff)
                #print(approx)
...
if __name__ == '__main__':
    tester = Tester('models_archive/model_weight5_satv2.pt', model.GNNSat_V2)
    ...
    if files:
        ...
        mus_new = np.zeros(cnf_data.n_clause + cnf_data.n_var)
        with OptUx(formula) as optux:
            mus_total = 0
            for mus in tqdm(optux.enumerate()):
                for i in mus:
                    mus_new[i+cnf_data.n_var-1] += 1
                mus_total += 1
                if mus_total == 100000:
                    break
        
        mus_new /= mus_total
        print(len(np.argwhere((mus_new > 0) & (mus_new < 1))))

        with MUSX(formula, solver=solver, verbosity=verbose) as musx:
            musx.compute(prob_arr=mus_new.tolist(), coeff=1)
