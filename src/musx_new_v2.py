...
class MUSX(object):
    ...
    def compute(self, pred=None, pred_real=None, coeff=1):
        """
        ...
        if not self.oracle.solve(assumptions=self.sels):
            ...
            approx = self.oracle.get_core() # No need to sort?
            use_pred = False
            if self.sels != pred:
                print("Difference in prediction size:", len(pred), len(self.sels), len(approx))
                if not self.oracle.solve(assumptions=pred):
                    #print("Changed core")
                    approx = sorted(self.oracle.get_core())
                else:
                    #print("Unchanged core(2)")
                    approx = sorted(approx)
                    use_pred = True
            else:
                #print("Unchanged core(1)")
                approx = sorted(approx)
                use_pred = True
            ...
            if use_pred:
                ...
                for i in range(len(approx)):
                    prob.append(pred_real[approx[i]-1] + (approx[i]-self.org_topv)/self.clause_amt*coeff)
                #print(approx)
                approx_gnn = np.asarray(approx)[np.argsort(prob)].tolist()

                #print("Min:",np.asarray(prob)[np.argsort(prob)][0])
                #print("Max:",np.asarray(prob)[np.argsort(prob)][-1])
                print("Reordering...")
            else:
                print("Using GNN core")
            ...
            self.deletion_start = time.time()
            # iterate over clauses in the approximation and try to delete them
            if use_pred:
                mus = self._compute_gnn(approx, approx_gnn)
            else:
                mus = self._compute(approx)

            self.deletion_fin = time.time()
...
if __name__ == '__main__':
    #tester = Tester('models_archive/model_2500.pt', model.GNNSat_V2)
        ...
        with MUSX(formula, solver=solver, verbosity=verbose) as musx:
            if gnn:
                if False:
                    ...
                    mus = musx.compute(pred=gnn_pred[1], pred_real=gnn_pred[0].tolist(), coeff=1)
            else:
