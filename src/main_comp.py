...
if __name__ == "__main__":
    ...
    TEST_EPOCHS = 10
    ITERATIONS = 10
    LOSS_WEIGHT_OFFSET = 10
    MODEL_SAVE_EPOCHS = 100
    ...
    if mode_type == 1 or mode_type == 2:
        if mode_type == 1:
            ...
            i = 1
            while os.path.exists("extracted_cores/unsat_cores_satcomp(0-" + str(i) + ").pickle"):
                with open("extracted_cores/unsat_cores_satcomp(0-" + str(i) + ").pickle", 'rb') as handle:
                    unsat_cores_train.update(pickle.load(handle))
                i += 1
            for cnf_file in tqdm(list(unsat_cores_train.keys())[1:]):
        ...
        test_path = "unsat_cores_satcomp(3-1).pickle"
        if test_type >= 0:
            test_path = "unsat_cores_satcomp(" + str(test_type) + ").pickle"
    ...
    if mode_type == 2:
        ...
        sum_unsat_correct = 0
        sum_correct = 0
        conf_mat_total = np.asarray([[-int(total_always_negative_test),0],[0,0]])
        ...
        print(pred[-100:].tolist())
        print(y_cpu.astype(pred.dtype)[-100:].tolist())
        print(np.sum(pred))
        print(np.sum(y_cpu))
        print(np.sum(pred-y_cpu))
        ...
        for epoch in range(1, EPOCHS+1):
            ...
            sum_correct = 0
            sum_unsat_correct = 0
            conf_mat_total = np.asarray([[-int(total_always_negative_train),0],[0,0]])
